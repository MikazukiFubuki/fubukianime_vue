<template>
  <section class="app-container">
    <!--搜索栏-->
    <el-input v-model="pagination.downloadType" placeholder="音乐下载类型" style="width: 200px;" class="filter-item" />
    ...
    <el-table size="big" current-row-key="id" :data="dataList" stripe highlight-current-row>
      ...
      <el-table-column prop="downloadType" label="音乐下载类型" align="center" />
    ...
    <!--新增表单-->
    <el-dialog title="新增音乐下载类型" :visible.sync="dialogFormVisible">
      ...
      <el-form ref="dataAddForm" :model="formData" :rules="rules" label-position="right" label-width="100px">
        ...
        <el-row>
          ...
          <el-col :span="24">
            ...
            <el-form-item label="音乐下载类型" prop="downloadType">

              <el-input v-model="formData.downloadType" />
    ...
    <!--编辑表单-->
    <el-dialog title="编辑音乐下载类型" :visible.sync="dialogFormVisible4Edit">
...
export default {
  data() {
    return {
      ...
      formData: {}, // 表单数据
      rules: {// 校验规则
        downloadType: [{ required: true, message: '音乐下载类型为必填项', trigger: 'blur' }]
      },
      pagination: {// 分页相关模型数据
        currentPage: 1, // 当前页码
