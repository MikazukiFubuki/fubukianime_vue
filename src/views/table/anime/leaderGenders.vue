  ...
  <section class="app-container">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="pagination" @submit.native.prevent>
        <el-form-item>
          <el-input v-model="pagination.leaderGender" placeholder="动画主角类型" style="width: 200px;" class="filter-item" />
        </el-form-item>
        <el-form-item>
    ...
    <el-table size="big" current-row-key="id" :data="dataList" stripe highlight-current-row>
      ...
      <el-table-column prop="leaderGender" label="动画主角类型" align="center" />
    ...
    <!--新增表单-->
    <el-dialog title="新增主角类型" :visible.sync="dialogFormVisible">
      <el-form ref="dataAddForm" :model="formData" :rules="rules" label-position="right" label-width="100px" >
        <el-row>
          <el-col :span="12">
            <el-form-item label="主角类型" prop="leaderGender">
              <el-input v-model="formData.leaderGender" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <!--编辑表单-->
    <el-dialog title="编辑主角类型" :visible.sync="dialogFormVisible4Edit">
...
import {
  ...
  edit,
  add, getId
} from '@/api/anime/leaderGenders.js'
...
export default {
  data() {
    return {
      ...
      formData: {}, // 表单数据
      rules: {// 校验规则
        leaderGender: [{ required: true, message: '动画主角类型为必填项', trigger: 'blur' }]
      },
      pagination: {// 分页相关模型数据
        currentPage: 1, // 当前页码
